...
KERNEL_FQ void m04540_mxx (KERN_ATTR_VECTOR ())
{
  ...
  // --- ЧИТАЕМ СОЛЬ ВРУЧНУЮ ---
  // Читаем соль в Little Endian (как она лежит в памяти)
  // Мы не используем update_global, чтобы точно знать, что происходит
  u32 s[4];
  ...
  // Длина соли + длина хеша (16 + 20 = 36)
  ...
  for (u32 il_pos = 0; il_pos < IL_CNT; il_pos += VECT_SIZE)
  {
    ...
    // Подготовка буфера для Step 2: [SALT] + [HASH1]
    // Соль (16 байт)
    u32x w_next[16]; // Буфер для следующего шага
    w_next[0] = s[0];
    w_next[1] = s[1];
    w_next[2] = s[2];
    w_next[3] = s[3];

    // Хеш (20 байт). Он в BE, нужно перевернуть в LE для sha1_update
    ...
    // Обновляем вручную собранным буфером длиной 36 байт
    sha1_update_vector_64 (&ctx2, w_next, w_next + 4, w_next + 8, w_next + 12, combined_len);
    sha1_final_vector (&ctx2);

    // Подготовка буфера для Step 3: [SALT] + [HASH2]
    // Соль уже на месте в w_next[0..3], обновляем только хеш
    ...
    // === COMPARE ===
    // Финальный переворот для сравнения с C-модулем
